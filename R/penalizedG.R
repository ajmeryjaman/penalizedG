...
#'   delta <- c(1, 1, 1.2, 1.2, -0.9, 0.8, -1, 1.3) # treatment-free model parameters
#'   psi <- c(1, 1.5, -1, -1.2, 1.5, 1.2, 0) # blip parameters
#' 
...
#'                                                    collapse = "+"), collapse=""))
#' 
#' treat.model <- ~l1+l2+l3+l4+l5+l6 # Treatment model is correctly specified
#' # treat.model <- ~1 # Use this to see what happens if the treatment model is misspecified
#' 
#' p <- length(model.matrix(tf.model, data=data.s)[1,])
#' 
#' ## penG() performs penalized G-estimation for a single value of tuning parameter lambda
#' ## We can see the estimates from the full model by setting lambda = 0.01 (a small value)
#' out <- penG(data = data.s, wc.str = "exchangeable", id.var="id", response.var="y",
#'             treat.var="a", tf.model=tf.model, treat.model = treat.model,
#'             lambda = 0.01, maxitr = 50, penalty = "SCAD")
#' out$nitr
#' round(out$estimate[1:7], 2)
#' 
